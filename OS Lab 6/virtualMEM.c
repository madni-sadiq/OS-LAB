...
int main(int argc, char * argv[]) {
	...
	if (argc == 2) {
       ...
       {	
       	...
       	pageno = (addr & 65280) >> 8; // extracting page number from logical address i.e. bit 15 to bit 8
    		offset = (addr & 255); // extracting offset from logical address i.e. bit 7 to bit 0
    		if (PageTable[pageno] == -1){// when page is not found in page table 
    		...
    		else{ // when page is availabe in page table 
